...
class DataQualitySection:
    ...
    def _compare_reports(
        ...
        """通用比對邏輯（與 AU-03 一致）。回傳 (consensus_type, direction, confidence, reasoning)。"""
        ...
        if same_direction and confidence_diff <= 0.2:
            ...
            reasoning = (
                f"蓉蓉: {report_a.reasoning} | 小方: {report_b.reasoning}"
            )
            ...
            reasoning = (
                f"方向一致但信心差異大: 蓉蓉 {report_a.sub_confidence:.2f} "
            ...
            reasoning = (
                f"大分歧採保守: 蓉蓉 {report_a.direction} | 小方 {report_b.direction}"
            )
            self.logger.warning(f"DM-02 雙人大分歧: {reasoning}")
