...
## 當前階段：Phase 5 — 進化層（✅ 完成，可進 Phase 6 整合測試）

Phase 5 的目標是建立進化層（Evolution Layer），讓系統具備自我驗證、基準比對、角色調整與策略回測能力。
...
- [x] **HindsightEngine `hindsight_engine.py`**（事後驗證引擎）— 統籌所有角色的 SelfReview 事後驗證；自動判斷 correct / incorrect / partial_correct；廣播 `hindsight.verified`；支援批次覆核與加權正確率統計；40 項測試
- [x] **阿柯 `BaselineComparator`（TO-03，毒舌基準比對員）** — 三條績效曲線對比（系統 vs Buy & Hold vs 零操作基準）；ETH 價格即時抓取；連敗 28 天觸發 OVERHAUL 快報（ANOMALY_FLASH, requires_ack）；毒舌四段評語（輸給 BH / 輸給零操作 / 抓不到 ETH 行情 / 勉強沒虧）；廣播 `baseline.comparison`；66 項測試（12 組）
- [x] **大劉 `EvolutionDirector`（TO-Manager，進化部經理）** — 角色準確率監控；加權準確率（correct + partial×0.5）< 0.50 觸發 ADJUSTMENT；< 0.35 升為人工建議 SUGGESTION；ADJUSTMENT 設 7 天冷卻期，自動追蹤效果；回滾判定（改善 < 0.05）；連續 5 次無效 → 自我降權至最低 0.5；61 項測試（10 組）
- [x] **`StrategyCoreBacktest`（策略核心回測，第一層）** — 1H ETHUSDT bar-by-bar 模擬，分批抓取 5,000 根歷史 K 線；指標：MA20/50/200、RSI14、ATR14；方向判斷含 MA200 趨勢過濾與 MA20 距離訊號；支援 `use_trend_filter` / `profit_loss_ratio_target` / `min_confidence` 三參數對比測試；58 項測試（10 組）

### 回測核心結論（2025-10-11 → 2026-05-05，ETH 熊市期間）

| 指標 | 基準方案結果 |
|------|------------|
| 資料範圍 | 5,000 根 1H K 線（約 208 天） |
| 總報酬 | **+5.1%**（ETH BH 同期 -37.7%） |
| vs Buy & Hold | **+42.9%**（風控有效，成功迴避熊市損失） |
| 最大回撤 | **9.5%**（< 15% 門檻，合格） |
| 期望值（EV） | 0.69（未達 EV > 1，需整合四課判斷） |

**策略方向正確，風控設計有效**。純技術指標（RSI+MA）在熊市的信號預測力不足（勝率僅 34.5%），整合四課（IO、CA、GA、TK）判斷後預期可顯著提升勝率，達成 EV > 1。

**進化層完成，可進入 Phase 6 整合測試。**
...
│   │   └── arbiter.py                   # 老王：仲裁者
│   └── evolution/                       # 進化層（Phase 5）
│       ├── hindsight_engine.py          # 事後驗證引擎
│       ├── baseline_comparator.py       # 阿柯：毒舌基準比對員
│       ├── evolution_director.py        # 大劉：進化部經理
│       └── strategy_core_backtest.py   # 策略核心回測（第一層）
...
# Phase 5 驗證（進化層）
python tests/test_hindsight_engine.py     # 事後驗證引擎（40 項）
python tests/test_baseline_comparator.py  # 阿柯 毒舌基準比對員（66 項）
python tests/test_evolution_director.py   # 大劉 進化部經理（61 項）
python tests/test_strategy_core_backtest.py # 策略核心回測（58 項）
```
